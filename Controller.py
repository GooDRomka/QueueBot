...
def send_text_controller(message):
    ...
    Comands = {
        ...
        "cоздать новый": "",
        "какой я в очереди": "my_position",
        "выбрать магазин": "choice_shop",
        "отменить запись": "cancel_record",
        "выйти": "finish",
    }
...
def answer_maker(User_name, _id, message=None):
    try:
        ...
        if flag == "work":
            keyboard1.row(
                "Выбрать магазин", "Отменить запись", "Какой я в очереди", "Выйти"
            )
        ...
        if flag == "cancel_record":
            ...
            if (user_login not in myQueue) or (len(myQueue[user_login].keys())) == 0:
                ...
                bot.send_message(
                    _id, f"Из какого магазина Вас убрать: {myQueue[user_login].keys()}"
                )
        ...
        if flag == "choice_shop":
            bot.send_message(
                _id, f"Ловите список магазинов, в какой записать Вас?: {[Shops.keys()]}"
            )
        ...
        if flag == "wait_choice_shop":
            if message in queue.keys():
                ...
                bot.send_message(
                    _id, "Все номера свободный, введите на какой номер Вас записать"
                )
                choose_shop = message
                return "wait_number_queue"
        if flag == "wait_number_queue":
            ...
            if choose_shop not in queue:
                ...
                bot.send_message(
                    _id, f"Вы записаны в {choose_shop}, порядок в очереди {message}"
                )
            elif message in queue[choose_shop].keys():
                bot.send_message(
                    _id, f"Номер занят, посмотри еще\n {freeNum(choose_shop)}"
                )
                return "wait_number_queue"
            else:
                queue[choose_shop][message] = user_login
...
def freeNum(choose_shop):
    ...
    return list(
        [
            set(range(1, max_number_clients))
            - set([int(item) for item in queue[choose_shop].keys()])
        ]
    )
